        ...
        <div className="flex-1 flex justify-center">
          <div className="grid grid-cols-3 gap-10 py-4">
            {awards.map((award, index) => (
              <div key={index} className={`bg-white rounded-2xl p-2 md:p-4 shadow-lg flex items-center justify-center h-24 w-24 md:h-32 md:w-32 transition-transform duration-300 hover:scale-105 ${index % 3 === 1 ? 'transform -translate-y-8' : ''}`}>
                <Image src={award.src} alt={award.alt} width={100} height={100} className="object-contain" />
              </div>
            ))}
          </div>
        </div>
